...
export default ({setQuiz, setTheme}) => {
    const [selected, setSelected] = useState(0);
    console.log(selected)
    return (
  <>
  <Wrapper>
    <Icon onClick={() => {setQuiz(futureThemes[0]);setTheme(futureThemes[0].theme);setSelected(0)}}>
      <Image src={(selected === 0) ? futureThemes[0].icon_selected : futureThemes[0].icon} />
    </Icon>
    <Icon onClick={() => {setQuiz(futureThemes[1]);setTheme(futureThemes[1].theme);setSelected(1)}}>
